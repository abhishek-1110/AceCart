...
import { addToCart2 } from "../../redux/actions/cartActions";
...
const ActionItem2 = ({ electronicsProduct }) => {
  const navigate = useNavigate(); // intialization
  const dispatch = useDispatch();
  const [quantity, setQuantity] = useState(1);
  // object desctructring
  const { id } = electronicsProduct;

  const addItemToCart = () => {
    dispatch(addToCart2(id, quantity));
    navigate("/cart");
  };
  ...
  return (
    <LeftContainer>
      ...
      >
        <Image src={electronicsProduct.detailUrl} alt="electronicsProduct" />
      </Box>
      <Box style={{ width: "100%" }}>
        <StyledButton
          variant="contained"
